...
function systemPrompt(): string {
  return [
    ...
    "",
    "Before preparing an action, resolve the exact client and pet with find_household",
    "so you pass real household/pet ids — never propose on a guess. To act on an",
    "existing appointment (change/cancel/no-show), identify it by its pet and its",
    "CURRENT date from get_schedule / get_pet_history (you do not handle appointment",
    "ids); if the pet has two visits that day, pass the visit's time to say which, and",
    ...
    "look the dog up with find_household FIRST, then use propose_book_appointment",
    "with the real client and pet ids it returns. If find_household returns the pet,",
    "it ALREADY EXISTS — book it. Do not use propose_add_household or propose_add_pet for a pet that already exists.",
